...
public class DefenseUpgrade extends UpgradeComponent implements IArmoryUpgrade{

	public DefenseUpgrade(UpgradePointSystem system, Clan clan, String displayName) {
		super(10, system, clan, displayName);
		this.type = ArmoryUpgradeType.DEFENSE;
	}
	...
	public void applyFor(Player player) {
		...
		UpgradeBuff buff = new UpgradeBuff(player, Stat.ARMOR, percentBuff);
		buff.setDisplayName("§eClan Buff: §f" + super.getDisplayName());
		buff.addDescLine("");
		buff.addDescLine("§f[§a+§f] Dieser Buff erhöht deine Rüstung");
		buff.addDescLine("§fum §e" + percentBuff + "%");
		buff.setIconMaterial(Material.IRON_CHESTPLATE);
